        ...
        if (displayRenderModeRegistry != null
            ...
            computerLanguageRegistry.registerObjectType(
                    ...
                    Collections.singleton(HTMLLikeParser.parseHTMLLikeParagraph(null, "Binding that tells method " +
                            "where to render text to. Usually passed as " +
                            "a parameter to methods of Text Graphics Card computer module.")));

            computerLanguageRegistry.registerObjectType(
                    "TextOffScreenBuffer",
                    Collections.singleton(HTMLLikeParser.parseHTMLLikeParagraph(null, "In memory buffer for text, " +
                            ...
                            "<h navigate:" + DocumentationBuilder.getObjectTypePageId("TextRenderBinding") + 
                            ">TextRenderBinding</h> " +
                            "is expected, as it can also be drawn text on.")));

            computerModuleRegistry.registerComputerModule(
                    TEXT_GRAPHICS_CARD_MODULE_TYPE,
                    new TextOnlyGraphicsCardComputerModule(multiBlockRegistry, TEXT_GRAPHICS_CARD_MODULE_TYPE, "Text " +
                            "Graphics Card"),
